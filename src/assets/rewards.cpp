...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for AIDP
    ...
    if (p_rewardSnapshot.strDistributionAsset != "AIDP") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is AIDP with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using AIDP differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "AIDP") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Aidp address (already validated during ownership snapshot creation)
