...
#include "aidpaddressvalidator.h"
#include "aidpunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Aidp address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new AidpAddressEntryValidator(parent));
    widget->setCheckValidator(new AidpAddressCheckValidator(parent));
}
...
bool parseAidpURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no aidp: URI
    if(!uri.isValid() || uri.scheme() != QString("aidp"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!AidpUnits::parse(AidpUnits::AIDP, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseAidpURI(QString uri, SendCoinsRecipient *out)
{
    // Convert aidp:// to aidp:
    //
    //    Cannot handle this later, because aidp:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("aidp://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "aidp:");
    }
    QUrl uriInstance(uri);
    return parseAidpURI(uriInstance, out);
}

QString formatAidpURI(const SendCoinsRecipient &info)
{
    QString ret = QString("aidp:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AidpUnits::format(AidpUnits::AIDP, info.amount, false, AidpUnits::separatorNever));
        paramCount++;
    }
...
bool openAidpConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(AIDP_CONF_FILENAME);
    ...
    /* Open aidp.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aidp.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Aidp (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Aidp (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "aidp.desktop";
    return GetAutostartDir() / strprintf("aidp-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a aidp.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Aidp\n";
        else
            optionFile << strprintf("Name=Aidp (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the aidp app
...
bool GetStartOnSystemStartup()
{
    CFURLRef aidpAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (aidpAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, aidpAppUrl);

    CFRelease(aidpAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add aidp app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, aidpAppUrl, nullptr, nullptr);
    }
