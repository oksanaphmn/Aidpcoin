...
const std::string AidpGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AidpGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(AidpGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

AidpGUI::AidpGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void AidpGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Aidp address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and aidp: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to AIDP addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Aidp addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Aidp addresses"));
    ...
    openAction->setStatusTip(tr("Open a aidp: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Aidp command-line options").arg(tr(PACKAGE_NAME)));
...
void AidpGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/aidptext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/aidpcointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* aidpLabelLayout = new QVBoxLayout(toolbarWidget);
        aidpLabelLayout->addWidget(labelToolbar);
        aidpLabelLayout->addWidget(m_toolbar);
        aidpLabelLayout->setDirection(QBoxLayout::TopToBottom);
        aidpLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Aidpcoin Market Price"));
        ...
        comboAidpUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboAidpUnit->addItems(list);
        comboAidpUnit->setFixedHeight(26);
        comboAidpUnit->setContentsMargins(5,0,0,0);
        comboAidpUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboAidpUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/AidpProject/Aidpcoin/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboAidpUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboAidpUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Aidpcoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/AidpProject/Aidpcoin/releases";
...
void AidpGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/aidptext")));
        m_toolbar->setMaximumWidth(65);
...
void AidpGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AidpGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AidpGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AidpGUI::setWalletActionsEnabled(bool enabled)
{
...
void AidpGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AidpGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AidpGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AidpGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AidpGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void AidpGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AidpGUI::openClicked()
{
...
void AidpGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AidpGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AidpGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AidpGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AidpGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AidpGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** AIDP START */
void AidpGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void AidpGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void AidpGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void AidpGUI::gotoRestrictedAssetsPage()
{
...
void AidpGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Aidp network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AidpGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AidpGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AidpGUI::updateHeadersSyncProgressLabel()
{
...
void AidpGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AidpGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Aidp"); // default title
...
void AidpGUI::changeEvent(QEvent *e)
{
...
void AidpGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AidpGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AidpGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "AIDP")
        msg += tr("Amount: %1\n").arg(AidpUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(AidpUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void AidpGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AidpGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AidpGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AidpGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AidpGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void AidpGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AidpGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AidpGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AidpGUI::detectShutdown()
{
...
void AidpGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AidpGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(AidpGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void AidpGUI::subscribeToCoreSignals()
{
...
void AidpGUI::unsubscribeFromCoreSignals()
{
...
void AidpGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void AidpGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<AidpUnits::Unit> units = AidpUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AidpUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(AidpUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(AidpUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (AidpUnits::Unit u : AidpUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AidpUnits::name(u)), this);
...
{
    setText(AidpUnits::name(newUnits));
}
...
void AidpGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AidpGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "AidpGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboAidpUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void AidpGUI::getPriceInfo()
{
    request->setUrl(QUrl(QString("https://api.coingecko.com/api/v3/simple/price?ids=aidp&vs_currencies=%1").arg(this->currentPriceDisplay->Ticker)));

    https://api.coingecko.com/api/v3/simple/price?ids=aidp&vs_currencies=
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void AidpGUI::mnemonic()
{
...
void AidpGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/AidpProject/Aidpcoin/releases"));
