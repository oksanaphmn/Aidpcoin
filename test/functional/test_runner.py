...
`test/functional/test_framework/test_framework.py:AidpTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/aidp_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and aidpd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_aidpd = config["components"].getboolean("ENABLE_AIDPD")
    if not (enable_wallet and enable_cli and enable_aidpd):
        print("No functional tests to run. Wallet, utils, and aidpd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if aidpd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "aidpd"]) is not None:
            print("%sWARNING!%s There is already a aidpd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "AIDPD" not in os.environ:
        os.environ["AIDPD"] = build_dir + '/src/aidpd' + exeext
        os.environ["AIDPCLI"] = build_dir + '/src/aidp-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `aidp-cli help` (`rpc_interface.txt`).
