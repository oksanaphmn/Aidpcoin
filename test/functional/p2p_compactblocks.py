...
# TestNode: A peer we use to send messages to aidpd, and store responses.
...
class CompactBlocksTest(AidpTestFramework):
    ...
    # This test actually causes aidpd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that aidpd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for aidpd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # aidpd will not send blocktxn responses for blocks whose height is
