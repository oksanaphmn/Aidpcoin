...
dnl Copyright (c) 2024 The AIDP Core developers
...
AC_DEFUN([AIDP_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build aidp-qt GUI (default=auto)])],
    [
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); aidp_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   AIDP_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _AIDP_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: aidp_enable_qt, aidp_enable_qt_dbus, aidp_enable_qt_test
AC_DEFUN([AIDP_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  AIDP_QT_CHECK([_AIDP_QT_FIND_LIBS])

  dnl This is ugly and complicated. Yuck. Works as follows:
  dnl For Qt5, we can check a header to find out whether Qt is build
  dnl statically. When Qt is built statically, some plugins must be linked into
  dnl the final binary as well.
  dnl _AIDP_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  if test "x$aidp_cv_static_qt" = xyes; then
    ...
    if test "x$TARGET_OS" != xandroid; then
      _AIDP_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _AIDP_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _AIDP_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _AIDP_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _AIDP_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  AIDP_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  AIDP_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  AIDP_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  AIDP_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  AIDP_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  ...
  if test x$aidp_enable_qt = xyes; then
    AC_MSG_RESULT([$aidp_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$aidp_enable_qt])
  fi
...
dnl
dnl _AIDP_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_AIDP_QT_CHECK_STATIC_PLUGIN], [
...
AC_DEFUN([_AIDP_QT_FIND_LIBS],[
  AIDP_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [AIDP_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  AIDP_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [AIDP_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  AIDP_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [AIDP_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  AIDP_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [AIDP_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
