...
The Aidp Core daemon can be configured to act as a trusted "border
router", implementing the aidp wire protocol and relay, making
...
The ZeroMQ feature in Aidp Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aidpd:
...
For instance:
    ...
    $ aidpd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/aidpd.tx.raw
...
These options can also be provided in aidp.conf.
...
From the perspective of aidpd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aidpd directly. Furthermore, no information is
...
using. Aidpd appends an up-counting sequence number to each
